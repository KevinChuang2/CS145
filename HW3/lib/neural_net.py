...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        def softmax_loss(x, y):
            ...
            logs = -np.log(prob[range(x.shape[0]), y])
            loss = np.sum(logs)/x.shape[0]
            dx = prob
            # subtract y (where y is 1)
        ...
        def MSE_loss(y_pred, y_target):
            ...
            hotArray = np.zeros((y_pred.shape[0], y_pred.shape[1]))
            hotArray[np.arange(y_pred.shape[0]),y_target]=1
            loss = np.sum((y_pred - hotArray)**2)/y_pred.shape[0]
            dy_pred = 2/y_pred.shape[0]*(y_pred - hotArray)
            # ================================================================ #
            # END YOUR CODE HERE
            # ================================================================ #
            return loss, dy_pred
